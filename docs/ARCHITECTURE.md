...
- **`src/main.ts`** — all DOM wiring. Renders the page shell once, then on
  ...
  the TESS link, and syncs the URL hash via `history.replaceState`.
...
boundaries (empty input, non-alphabetic characters, the 3-letter
insufficient-signal threshold). `tests/fixtures/brandNames.ts` holds a
reference set of 20 real names and 20 invented pseudo-brands;
`tests/fixtures.test.ts` asserts `analyzeBrandName` verdicts at least 80%
of each group correctly (currently 100% real / 90% invented). DOM wiring
in `main.ts` is intentionally thin and not unit-tested — it was verified
by driving a real Chromium instance against the dev server and a
